...
// Project: https://github.com/ShiftLeft Inc/sarif-sdk
...
`;
            ...
            if (
                prop.type === "object" &&
                prop.additionalPropsType !== undefined &&
...
`;
                ...
                output += `${Writer.outputAdditionalProps(
                    prop.additionalPropsType
                )}};`;
            } else {
...
export namespace ${sClass.name} {${enums}
    ...
    private static createDocumentation(
        ...
        for (const msgPiece of msgSplit) {
            if (
                spaces + tempLine.length + 1 /*space*/ + msgPiece.length >=
                120
            ) {
                output += `
${tempLine}`;
