...
export class FileMapper implements Disposable {
  ...
  public static readonly MapCommand = "extension.shiftleft.Map";

  /**
   * Contains a mapping of root paths (base paths) in the SARIF
   * to local paths.
   */
  private readonly baseRemapping: Map<string, string> = new Map<
    string,
    string
  >();
   ...
   */
  private readonly fileRemapping: Map<string, Uri | undefined> = new Map<
    string,
    Uri | undefined
  >();
   ...
   */
  private readonly fileIndexKeyMapping: Map<string, string> = new Map<
    string,
    string
  >();

  private readonly mappingChangedEventEmitter: EventEmitter<
    Uri
  ...
  public constructor() {
    ...
    this.disposables.push(
      workspace.onDidChangeConfiguration(this.updateRootPaths, this)
    );
    this.disposables.push(
      commands.registerCommand(
        FileMapper.MapCommand,
        this.mapFileCommand.bind(this)
      )
    );
  }
   ...
   * @param uriBase the base path of the uri
   */
  public async getUserToChooseFile(
    origUri: Uri,
    uriBase?: string
  ): Promise<void> {
    const oldProgressMsg: string | undefined =
      ProgressHelper.Instance.CurrentMessage;
    await ProgressHelper.Instance.setProgressReport("Waiting for user input");

    const remapResult: string | undefined = await this.openRemappingInputDialog(
      origUri
    );
    ...
    if (!remapResult) {
      this.addToFileMapping(
        Utilities.getFsPathWithFragment(origUri),
        undefined
      );
      return;
    }
  ...
  public async map(uri: Uri, uriBase?: string): Promise<void> {
    ...
    const existingFileMapping: Uri | undefined = this.fileRemapping.get(
      uriPath
    );
  ...
  public async mapArtifacts(
    ...
    for (const [artifactIndex, artifact] of artifacts.entries()) {
      const fileLocation: sarif.ArtifactLocation | undefined =
        artifact.location;
      ...
      const uriBase: string | undefined = Utilities.getUriBase(
        runInfo,
        fileLocation
      );
      const uriWithBase: Uri = Utilities.combineUriWithUriBase(
   ...
   */
  private async openRemappingInputDialog(
    uri: Uri
        ...
        input.onDidTriggerButton(async (button) => {
          switch ((<RemappingQuickInputButtons>button).remappingType) {
            case "Open":
              const selectedUris:
                | Uri[]
                | undefined = await window.showOpenDialog({
                canSelectMany: false,
                openLabel: "Map",
              });
        ...
        input.onDidChangeValue(() => {
          ...
          if (directory && directory.length !== 0) {
            ...
            if (validateUri) {
              ...
              if (isDirectory) {
                const rootPathIndex: number = this.rootpaths.indexOf(
                  validateUri.fsPath
                );
                if (rootPathIndex !== -1) {
  ...
  private saveBasePath(
    ...
    if (uriBase !== undefined) {
      const relativePath: string = oPath.substring(
        oPath.indexOf(uriBase) + uriBase.length
      );
      const index: number = rPath.indexOf(relativePath);
      if (index !== -1) {
        this.baseRemapping.set(
          oPath.replace(relativePath, ""),
          rPath.replace(relativePath, "")
        );
        return;
      }
    }
    ...
    for (let i: number = 1; i <= rPath.length; i++) {
      ...
      if (
        oIndex === 0 ||
        oPath[oIndex].toLocaleLowerCase() !== rPath[rIndex].toLocaleLowerCase()
      ) {
        this.baseRemapping.set(
          oPath.substring(0, oIndex + 1),
          rPath.substring(0, rIndex + 1)
        );
        break;
      }
    }
  ...
  private tryConfigRootpathsUri(uri: Uri, uriBase?: string): boolean {
    const originPath: path.ParsedPath = path.parse(
      Utilities.getFsPathWithFragment(uri)
    );
    ...
    for (const rootpath of this.rootpaths) {
      ...
      while (dirParts.length !== 0) {
        const mappedUri: Uri = Uri.file(
          Utilities.joinPath(rootpath, dirParts.join(path.sep))
        );
        if (this.tryMapUri(mappedUri, Utilities.getFsPathWithFragment(uri))) {
  ...
  private updateRootPaths(event?: ConfigurationChangeEvent): void {
    if (!event || event.affectsConfiguration(Utilities.configSection)) {
      ...
      const newRootPaths: string[] = sarifConfig
        .get(ConfigRootPaths, [])
  ...
  private updateMappingsWithRootPaths(): void {
    ...
    this.fileRemapping.forEach(
      (
        value: Uri | undefined,
        key: string,
        map: Map<string, Uri | undefined>
      ) => {
        remapped =
          remapped || this.tryConfigRootpathsUri(Uri.file(key), undefined);
      }
    );
