...
export namespace ResultInfoFactory {
  ...
  export async function create(
    ...
    const locations: Location[] = await ResultInfoFactory.parseLocations(
      fileMappper,
      runInfo,
      result.locations
    );
    const relatedLocations: Location[] = await ResultInfoFactory.parseLocations(
      fileMappper,
      runInfo,
      result.relatedLocations
    );
    const attachments: Attachment[] = await parseAttachments(
      fileMappper,
      runInfo,
      result.attachments
    );
    const fixes: Fix[] = await parseFixes(fileMappper, runInfo, result.fixes);
    const codeFlows: CodeFlow[] = await CodeFlowFactory.create(
      fileMappper,
      runInfo,
      result.codeFlows
    );
    const stacks: Stacks = await parseStacks(
      fileMappper,
      runInfo,
      result.stacks
    );
      ...
      if (ruleDescriptors && ruleDescriptors[ruleIndex]) {
        ...
        if (result.message && rule.messageStrings) {
          ...
          if (resultMsgId && rule.messageStrings) {
            const sarifMessageString: sarif.MultiformatMessageString =
              rule.messageStrings[resultMsgId];
  ...
  export async function parseLocations(
    ...
    runInfo: RunInfo,
    sarifLocations: sarif.Location[] | undefined
  ): Promise<Location[]> {
    ...
    if (sarifLocations) {
      for (const sarifLocation of sarifLocations) {
        locations.push(
          await LocationFactory.create(fileMapper, runInfo, sarifLocation)
        );
      }
    } else {
  ...
  async function parseAttachments(
    fileMapper: FileMapper,
    runInfo: RunInfo,
    sarifAttachments: sarif.Attachment[] | undefined
  ): Promise<Attachment[]> {
    if (!sarifAttachments) {
      return [];
    }
    ...
    for (const sarifAttachment of sarifAttachments) {
      const description: Message = Utilities.parseSarifMessage(
        sarifAttachment.description
      );
      ...
      if (sarifAttachment.regions) {
        for (const sarifRegion of sarifAttachment.regions) {
          regionsOfInterest.push(
            await LocationFactory.create(fileMapper, runInfo, {
              physicalLocation: {
                artifactLocation: sarifAttachment.artifactLocation,
                region: sarifRegion,
              },
   ...
   */
  async function parseFixes(
    fileMapper: FileMapper,
    runInfo: RunInfo,
    sarifFixes: sarif.Fix[] | undefined
  ): Promise<Fix[]> {
    if (!sarifFixes) {
      return [];
    }
        ...
        for (const sarifChange of sarifFix.artifactChanges) {
          ...
          if (sarifChange.replacements) {
            for (const sarifReplacement of sarifChange.replacements) {
              fixChanges.push({
                delete: LocationFactory.parseRange(
                  sarifReplacement.deletedRegion
                ).range,
                insert:
                  sarifReplacement.insertedContent &&
                  sarifReplacement.insertedContent.text,
   ...
   */
  async function parseStacks(
    fileMapper: FileMapper,
    runInfo: RunInfo,
    sarifStacks: sarif.Stack[] | undefined
  ): Promise<Stacks> {
    ...
    for (const sarifStack of sarifStacks) {
      ...
      for (const sarifFrame of sarifStack.frames) {
        ...
        const frameLocation: Location = await LocationFactory.create(
          fileMapper,
          runInfo,
          sarifFrame.location
        );
  ...
  function checkFrameContent(
    ...
    columnsWithContent.message =
      columnsWithContent.message &&
      frame.message.text !== undefined &&
      frame.message.text !== "";
    columnsWithContent.name =
      columnsWithContent.name && frame.name !== undefined && frame.name !== "";
    columnsWithContent.location =
      columnsWithContent.location && frame.location.range.start.line !== 0;
    columnsWithContent.filename =
      columnsWithContent.filename &&
      frame.location.fileName !== undefined &&
      frame.location.fileName !== "";
    columnsWithContent.parameters =
      columnsWithContent.parameters && frame.parameters.length !== 0;
    columnsWithContent.threadId =
      columnsWithContent.threadId && frame.threadId !== undefined;
