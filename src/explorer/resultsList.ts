...
import {
  ResultsListBaselineValue,
  ResultsListData,
  ResultsListGroup,
  ResultsListKindValue,
  ResultsListRow,
  ResultsListSeverityValue,
  ResultsListValue,
  DiagnosticData,
...
export class ResultsList {
  ...
  public constructor(explorer: ExplorerWebview) {
    ...
    const sevEle: HTMLDivElement = this.webview.createElement("div", {
      className: "severityiconwrapper",
    });
    ...
    this.severityIconHTMLEles.set(
      "warning",
      <HTMLElement>sevEle.cloneNode(true)
    );
  ...
  public set Data(value: ResultsListData) {
    if (
      ...
      (this.collapsedGroups[0] !== value.groupBy ||
        this.data.resultCount !== value.resultCount)
    ) {
  ...
  public updateSelection(): void {
    ...
    const id: string = JSON.stringify({
      resultId: diag.resultInfo.id,
      runId: diag.resultInfo.runId,
    });
    const curSelected: HTMLCollectionOf<Element> = document.getElementsByClassName(
    ...
    const rows: HTMLCollectionOf<HTMLElement> = <HTMLCollectionOf<HTMLElement>>(
      document.getElementsByClassName("listtablerow")
    );
  ...
  private createResultsListHeader(): void {
    const header: HTMLDivElement = <HTMLDivElement>(
      document.getElementById("resultslistheader")
    );
    ...
    header.appendChild(
      this.webview.createElement("label", {
    ...
    header.addEventListener(
      "click",
      this.webview.onHeaderClicked.bind(this.webview)
    );
  }
  ...
  private createResultsListPanelButtons(): void {
    const buttonBar: HTMLElement | null = document.getElementById(
    ...
    if (buttonBar && buttonBar.children.length === 0) {
      ...
      const showColButton: HTMLElement = this.webview.createElement("select", {
        ...
        id: "resultslistshowcol",
        tooltip: "Show or hide Columns",
      }) as HTMLSelectElement;
      showColButton.appendChild(
        this.webview.createElement("option", {
      ...
      const toggleFilterButtonContent: HTMLElement = this.webview.createElement(
        "span",
        {
      ...
      toggleFilterButtonContent.addEventListener(
        "click",
        this.onToggleFilterInput.bind(this)
      );
      const toggleFilterButtonIcon: HTMLElement = this.webview.createElement(
        "span",
        {
          className: "svgIconMasking",
      ...
      buttonBar.appendChild(
        this.webview.createElement("label", {
          attributes: { for: "resultslistgroupby" },
          id: "resultslistgroupbylabel",
          text: "Group by: ",
        })
      );
      const groupByButton: HTMLSelectElement = this.webview.createElement(
        "select",
        {
          id: "resultslistgroupby",
          tooltip: "Group by",
        }
      );
      groupByButton.appendChild(
        this.webview.createElement("option", {
          attributes: { disabled: null, hidden: null },
        })
      );
      groupByButton.addEventListener(
        "change",
        this.onGroupByChanged.bind(this)
      );
      buttonBar.appendChild(groupByButton);

      // Filter Input container
      const filterInputContainer: HTMLFormElement = this.webview.createElement(
        "form",
        {
          className: "hidden",
      ...
      const filterInput: HTMLInputElement = this.webview.createElement(
        "input",
        {
          attributes: {
            placeholder: "Filter. Eg: text",
            required: "",
            type: "text",
          },
      ...
      filterInputContainer.addEventListener("reset", () => {
        (document.getElementById(
          "resultslistfilterinput"
        ) as HTMLInputElement).focus();
        setTimeout(this.onFilterInput.bind(this), 0);
      });

      const filterInputClear: HTMLButtonElement = this.webview.createElement(
        "button",
        {
      ...
      const filterInputCaseMatchButton: HTMLButtonElement = this.webview.createElement(
        "button",
        {
          attributes: {},
      ...
      filterInputCaseMatchButton.addEventListener(
        "click",
        this.onToggleFilterCaseMatch.bind(this)
      );
  ...
  private createTableBody(columns: HTMLCollection): HTMLBodyElement {
    ...
    for (
      ...
      groupIndex < this.data.groups.length;
      groupIndex++
    ) {
      ...
      const groupRow: HTMLTableRowElement = this.createTableGroupRow(
        columns,
        groupIndex,
        group,
        groupState
      ...
      const rows: DocumentFragment = this.createTableResultRows(
        columns,
        group.rows,
        groupIndex,
        groupState
   ...
   */
  private createTableGroupRow(
    cols: HTMLCollection,
    groupId: number,
    group: ResultsListGroup,
    state: ToggleState
  ): HTMLTableRowElement {
    ...
    if (this.data.groupBy === "severityLevel") {
      const sevInfo: TextAndTooltip = this.webview.severityTextAndTooltip(
        <sarif.Result.level>groupText
      );
      groupText = sevInfo.text;
      groupTooltip = sevInfo.tooltip;
    } else if (this.data.groupBy === "baselineState") {
      const baselineState: TextAndTooltip = this.webview.baselineStateTextAndTooltip(
        <sarif.Result.baselineState>groupText
      );
      groupText = baselineState.text;
      groupTooltip = baselineState.tooltip;
    }

    const groupCell: HTMLTableDataCellElement = this.webview.createElement(
      "th",
      {
  ...
  private createTableHeader(): HTMLHeadElement {
    ...
    for (const colName in this.data.columns) {
      if (!this.data.columns[colName].hide && this.data.groupBy !== colName) {
        const cell: HTMLTableHeaderCellElement = this.webview.createElement(
          "th",
          {
            attributes: {
              "data-name": colName,
  ...
  private createTableResultRows(
    ...
    const resultRowBase: HTMLTableRowElement = this.webview.createElement(
      "tr",
      {
        attributes: { "data-group": groupId, tabindex: "0" },
    ...
    const rowCellBase: HTMLTableDataCellElement = this.webview.createElement(
      "td"
    );

    for (const row of rows) {
      const resultRow: HTMLTableRowElement = <HTMLTableRowElement>(
        resultRowBase.cloneNode()
      );
      resultRow.dataset.id = JSON.stringify({
        resultId: row.resultId.value,
      ...
      if (
        diag &&
        diag.resultInfo.runId === row.runId.value &&
        diag.resultInfo.id === row.resultId.value
      ) {
        resultRow.classList.add("selected");
      }

      const iconCell: HTMLTableCellElement = <HTMLTableCellElement>(
        rowCellBase.cloneNode()
      );
      iconCell.classList.add("severityiconcell");
      const severityIcon:
        | HTMLElement
        | undefined = this.severityIconHTMLEles.get(row.severityLevel.value);
      if (!severityIcon) {
      ...
      for (let index: number = 1; index < cols.length; index++) {
        const col: HTMLTableHeaderCellElement = <HTMLTableHeaderCellElement>(
          cols[index]
        );
        const columnName: string | undefined = col.dataset.name;

        const colData: ResultsListValue | undefined =
          columnName !== undefined ? row[columnName] : undefined;
        ...
        if (colData) {
          ...
          if (
            (colData as ResultsListSeverityValue).customOrderType ===
              "Severity" &&
            colData.value !== undefined
          ) {
            textAndTooltip = this.webview.severityTextAndTooltip(colData.value);
          } else if (
            (colData as ResultsListBaselineValue).customOrderType ===
              "Baseline" &&
            colData.value !== undefined
          ) {
            textAndTooltip = this.webview.baselineStateTextAndTooltip(
              colData.value
            );
          } else if (
            (colData as ResultsListKindValue).customOrderType === "Kind" &&
            colData.value !== undefined
          ) {
        ...
        const rowCell: HTMLTableDataCellElement = <HTMLTableDataCellElement>(
          rowCellBase.cloneNode()
        );
  ...
  private onFilterInput(event: Event): void {
    const filterText: string = (<HTMLInputElement>(
      document.getElementById("resultslistfilterinput")
    )).value;
    this.webview.sendMessage({
      data: filterText,
      type: MessageType.ResultsListFilterApplied,
    });

    const filterIconContainer: HTMLElement | null = document.getElementById(
      "resultslistfilterbuttoncontainer"
    );
    ...
    if (filterText !== "") {
      ...
      filterIconContainer.classList.contains(activatedClass) === true
    ) {
      filterIconContainer.classList.remove(activatedClass);
      filterIconContainer.title = filterIconContainer.title.replace(
        activeTooltip,
        ""
      );
    }
  ...
  private onGroupByChanged(event: Event): void {
    ...
    this.webview.sendMessage({
      data: col,
      type: MessageType.ResultsListGroupChanged,
    });
  }
  ...
  private onRowClicked(event: Event): void {
    ...
    const curSelected: HTMLCollectionOf<Element> = row.parentElement.getElementsByClassName(
      "listtablerow selected"
    );
    while (curSelected.length > 0) {
      curSelected[0].classList.remove("selected");
  ...
  private onShowChanged(event: Event): void {
    ...
    this.webview.sendMessage({
      data: option.value,
      type: MessageType.ResultsListColumnToggled,
    });
  }
  ...
  private onToggleFilterCaseMatch(event: Event): void {
    ...
    this.webview.sendMessage({
      data: "",
      type: MessageType.ResultsListFilterCaseToggled,
    });
  }
  ...
  private onToggleFilterInput(event: Event): void {
    ...
    const filterInput: HTMLInputElement = <HTMLInputElement>(
      document.getElementById("resultslistfilterinputcontainer")
    );
    if (filterInput.classList.contains(hiddenClass) === true) {
      filterInput.classList.remove(hiddenClass);
      const htmlElementToFocus: HTMLElement | null = document.getElementById(
        "resultslistfilterinput"
      );
  ...
  private onSortClicked(event: Event): void {
    const col: string | undefined = (<HTMLTableHeaderCellElement>(
      event.srcElement
    )).dataset.name;
    if (col === undefined) {
    ...
    this.webview.sendMessage({
      data: col,
      type: MessageType.ResultsListSortChanged,
    });
  }
  ...
  private populateResultsListTable(): void {
    ...
    $("#resultslisttable").colResizable({ disable: false });

    // remove the rows of the table
    const table: HTMLTableElement = <HTMLTableElement>(
      document.getElementById("resultslisttable")
    );
    ...
    const tableBody: HTMLBodyElement = this.createTableBody(
  ...
  private toggleAllGroups(stateToToggle: ToggleState): void {
    ...
    const groups: HTMLCollectionOf<HTMLTableRowElement> = <
  ...
  private toggleGroup(row: HTMLTableRowElement): void {
    ...
    if (row.classList.contains(`${ToggleState.expanded}`)) {
      ...
      this.collapsedGroups.splice(
        this.collapsedGroups.indexOf(row.dataset.group),
        1
      );
    }

    const results: NodeListOf<Element> = document.querySelectorAll(
      "#resultslisttable > tbody > .listtablerow"
    );
  ...
  private updateResultsListPanelButtons(): void {
    const groupByButton: HTMLElement | null = document.getElementById(
      "resultslistgroupby"
    );
    const showColButton: HTMLElement | null = document.getElementById(
      "resultslistshowcol"
    );
    ...
    if (groupByButton.children.length === 1) {
      for (const col in this.data.columns) {
        if (this.data.columns.hasOwnProperty(col)) {
          const groupOption: HTMLOptionElement = this.webview.createElement(
            "option",
            {
              attributes: { value: col },
              text: this.data.columns[col].title,
            }
          );

          const showOption: HTMLOptionElement = <HTMLOptionElement>(
            groupOption.cloneNode(true)
          );
      ...
      if (this.data.filterText !== "") {
        (document.getElementById(
          "resultslistfilterinput"
        ) as HTMLInputElement).value = this.data.filterText;
        ...
        if (
          filterIconContainer &&
          filterIconContainer.classList.contains("activated") !== true
        ) {
    ...
    for (
      let index: number = 1;
      index < groupByButton.children.length;
      index++
    ) {
      const groupOption: HTMLOptionElement = <HTMLOptionElement>(
        groupByButton.children[index]
      );
    ...
    const caseButton: HTMLButtonElement = <HTMLButtonElement>(
      document.getElementById("filterinputcasebutton")
    );
