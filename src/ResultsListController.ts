...
import {
  ResultInfo,
  ResultsListColumn,
  ResultsListCustomOrderValue,
  ResultsListData,
  ResultsListGroup,
  ResultsListPositionValue,
  ResultsListRow,
  ResultsListSortBy,
  ResultsListValue,
  WebviewMessage,
...
import {
  SVDiagnosticCollection,
  SVDiagnosticsChangedEvent,
} from "./SVDiagnosticCollection";
...
export class ResultsListController implements Disposable {
  ...
  public constructor(
    ...
    this.disposables.push(
      workspace.onDidChangeConfiguration(this.onSettingsChanged, this)
    );
    this.disposables.push(
      explorerController.onWebViewMessage(this.onResultsListMessage.bind(this))
    );
    this.disposables.push(
      diagnosticCollection.diagnosticCollectionChanged(
        this.onDiagnosticCollectionChanged.bind(this)
      )
    );
  }
   ...
   */
  public updateResultsListData(
    diagnosticsChangedEvent: SVDiagnosticsChangedEvent
  ): void {
    ...
    if (diagnosticsChangedEvent.type === "Remove") {
      ...
      for (const key of diagnosticsChangedEvent.diagnostics.keys()) {
        const row: ResultsListRow = this.createResultsListRow(
          diagnosticsChangedEvent.diagnostics[key].resultInfo
        );
  ...
  public async onResultsListMessage(msg: WebviewMessage): Promise<void> {
    ...
    switch (msg.type) {
      case MessageType.ResultsListColumnToggled:
        const hideColsConfig: string[] = sarifConfig.get(
          this.configHideColumns,
          ["baselineState", "rank", "logicalLocation", "resultStartPos"]
        );
      ...
      case MessageType.ResultsListGroupChanged:
        let groupByConfig: string | undefined = sarifConfig.get(
          this.configGroupBy
        );
      ...
      case MessageType.ResultsListResultSelected:
        ...
        const diagnostic:
          ...
          | undefined = this.diagnosticCollection.getResultInfo(
          id.resultId,
          id.runId
        );
        if (!diagnostic) {
        ...
        const diagLocation: Location | undefined =
        ...
        if (diagLocation && diagLocation.uri) {
          ...
          textEditor.selection = new Selection(
            diagLocation.range.start,
            diagLocation.range.start
          );
          await this.codeActionProvider.provideCodeActions(
            textDocument,
            diagLocation.range,
      ...
      case MessageType.ResultsListSortChanged:
        const sortByConfig: ResultsListSortBy = sarifConfig.get(
          this.configSortBy,
          {
            ascending: true,
            column: msg.data,
  ...
  private checkIfGroupByChanged(sarifConfig: WorkspaceConfiguration): boolean {
    let changed: boolean = false;
    const group: string = sarifConfig.get(
      this.configGroupBy,
      ResultsListController.defaultGroupBy
    );
  ...
  private checkIfSortByChanged(sarifConfig: WorkspaceConfiguration): boolean {
    let changed: boolean = false;
    const sort: ResultsListSortBy = sarifConfig.get(
      this.configSortBy,
  ...
  private createResultsListRow(resultInfo: ResultInfo): ResultsListRow {
    const run: RunInfo | undefined = this.diagnosticCollection.getRunInfo(
      resultInfo.runId
    );
    ...
    switch (resultInfo.baselineState) {
      case "absent":
        baselineOrder = BaselineOrder.absent;
        break;
      case "new":
        baselineOrder = BaselineOrder.new;
        break;
      case "unchanged":
        baselineOrder = BaselineOrder.unchanged;
        break;
      case "updated":
        baselineOrder = BaselineOrder.updated;
        break;
    }
    ...
    switch (resultInfo.kind) {
      case "fail":
        kindOrder = KindOrder.fail;
        break;
      case "notApplicable":
        kindOrder = KindOrder.notApplicable;
        break;
      case "open":
        kindOrder = KindOrder.open;
        break;
      case "pass":
        kindOrder = KindOrder.pass;
        break;
      case "review":
        kindOrder = KindOrder.review;
        break;
    }
    ...
    switch (resultInfo.severityLevel) {
      case "error":
        sevOrder = SeverityLevelOrder.error;
        break;
      case "warning":
        sevOrder = SeverityLevelOrder.warning;
        break;
      case "none":
        sevOrder = SeverityLevelOrder.none;
        break;
      case "note":
        sevOrder = SeverityLevelOrder.note;
        break;
    }
    ...
    if (resultInfo.locations[0]) {
      ...
      if (loc.uri) {
        ...
        startPositionString = `(${startPosition.line + 1}, ${
  ...
  private applyFilterToRow(row: ResultsListRow, regExp: RegExp): boolean {
    if (
      (row["automationCat"].value && regExp.test(row["automationCat"].value)) ||
  ...
  private getResultData(): ResultsListData {
    ...
    const groups: Map<string, ResultsListGroup> = new Map<
      string,
      ResultsListGroup
    >();
    for (const postFilterRow of this.postFilterListRows) {
      const row: ResultsListRow | undefined = this.resultsListRows.get(
        postFilterRow
      );
      if (!row) {
        continue;
    ...
    for (const group of data.groups) {
      group.rows.sort((a, b) => {
        ...
        if (valueA === undefined || valueA.value === undefined) {
          ...
          (valueA as ResultsListPositionValue).pos !== undefined &&
          (valueB as ResultsListPositionValue).pos !== undefined
        ) {
          ...
          (valueA as ResultsListCustomOrderValue).order !== undefined
        ) {
          comp =
            (valueA as ResultsListCustomOrderValue).order -
  ...
  private onDiagnosticCollectionChanged(
    diagnosticChangeEvent: SVDiagnosticsChangedEvent
  ): void {
